...
# candidate's name on the first line. This pattern only fires as a fallback (see
# below) and only against that first non-blank line, so ordinary body text (e.g.
# "Skills: Python, SQL") is never touched - it requires 2-4 bare Title-Case
# words with no punctuation, which a skills/summary line won't match. This can
# still misfire on a name-shaped title-only header (e.g. "Professional Profile"
# on its own line), but that false positive only costs a decorative header
# line, versus the alternative of a real name leaking into the "blind" Judge
# prompt - an easy trade to make.
_NAME_LIKE_HEADER_RE = re.compile(r"^[A-Z][a-zA-Z'-]*(?:\s+[A-Z][a-zA-Z'-]*){1,3}$")


def _redact_probable_name_header(text: str) -> str:
    lines = text.split("\n")
    for i, line in enumerate(lines):
        stripped = line.strip()
        if not stripped:
            continue
        if _NAME_LIKE_HEADER_RE.fullmatch(stripped):
            lines[i] = line.replace(stripped, "[REDACTED NAME]")
        break
    return "\n".join(lines)
...
def redact_identity(raw_cv_text: str, contact: ContactInfo) -> str:
    ...
    if contact.name:
        ...
        text = _redact_probable_name_header(text)
    if contact.location:
